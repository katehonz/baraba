...
proc generatePurchaseLedger*(self: var VatManager, db: DbConn): string =
  ...
  for entry in sortedEntries:
    ...
    if entry.counterpart_id.isSome:
      let cp = findUuid(Counterpart, entry.counterpart_id.get, db)
      if cp.isSome:
        cpName = cp.get.name
        cpCity = cp.get.city
...
proc generateVatDeclaration*(self: var VatManager, db: DbConn): string =
  ...
  # Representative info
  let representativeEgn = formatField(self.company.representative_eik, 10)
  let representativeName = formatField(self.company.representative_name, 50)
...
proc generateVatFiles*(company: Company, period: string, db: DbConn): tuple[purchase: string, sales: string, deklar: string] =
  ...
  let allEntries = findWhereUuid(JournalEntry, db, "company_id = $1::uuid AND document_date >= $2::date AND document_date <= $3::date", company.id, startDateStr, endDateStr)
