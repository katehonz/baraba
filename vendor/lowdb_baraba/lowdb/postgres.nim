...
proc parseDate1(s: string): DateTime {.tags: [TimeEffect].} =
  # Parse PostgreSQL timestamp format: 2019-09-03 12:18:20.022531+02
  # Handles fractional seconds and short timezone format (+02 instead of +0200)
  ...
  try:
    ...
    # Parse time part: HH:MM:SS (starts at position 11)
    var hour = 0
    var minute = 0
    var second = 0

    if s.len >= 19:
      hour = parseInt(s[11..12])
      minute = parseInt(s[14..15])
      second = parseInt(s[17..18])

    result = dateTime(year, Month(month), day, hour, minute, second, zone = utc())
  except:
    result = now()
